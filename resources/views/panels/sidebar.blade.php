...
<div class="main-menu menu-fixed {{(($configData['theme'] === 'dark') || ($configData['theme'] === 'semi-dark')) ? 'menu-dark' : 'menu-light'}} menu-accordion menu-shadow" data-scroll-to-active="true">
        ...
        <ul class="nav navbar-nav flex-row">
            <li class="nav-item me-auto">
                <a class="navbar-brand" href="{{url('/')}}">
                    <h2 class="brand-text">Placement Cell</h2>
                </a>
            </li>
    ...
    <div class="main-menu-content">
        <ul class="navigation navigation-main nav-pills nav-pill-danger" id="main-menu-navigation" data-menu="menu-navigation">
                        ...
                        <li class="nav-item {{ $custom_classes }} {{Route::currentRouteName() === $menu->slug ? 'active' : ''}}">
                            <a href="{{isset($menu->url)? url($menu->url):'javascript:void(0)'}}" class="d-flex align-items-center"
                               target="{{isset($menu->newTab) ? '_blank':'_self'}}">
                                <i class="{{ $menu->icon }}" style="font-size: 18px; height: 20px"></i>
                                <span class="menu-title text-truncate" style="font-weight: bold">{{ __('locale.'.$menu->name) }}</span>
                                @if (isset($menu->badge))
